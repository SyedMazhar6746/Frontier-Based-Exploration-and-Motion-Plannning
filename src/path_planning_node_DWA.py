...
class OnlinePlanner:
    ...
    def __init__(self, gridmap_topic, odom_topic, cmd_vel_topic, dominion, distance_threshold):
        ...
        self.pub_trajectory = rospy.Publisher('trajectory_path', MarkerArray, queue_size=10) 
    ...
    def controller(self, event):
        ...
        if self.path is not None and len(self.path) > 0:
            ...
            if np.linalg.norm(self.path[0][0:2] - self.current_pose[0:2]) < 4*self.svc.resolution  and np.linalg.norm(self.path[0][0:1] != self.goal): 
                ...
                v, w, trajectory = compute_dynamic_window(self.current_pose, self.current_v, self.current_w, self.path, self.svc) 
                self.goal_reached = False
                goal_reached_msg = Bool()
                goal_reached_msg.data = self.goal_reached
    ...
    def dwa_path(self, trajectory): 
        marker_msg = Marker(
            header=rospy.Header(frame_id='world'),  # Set the frame ID of the marker
            id=0,
            type=Marker.LINE_STRIP, 
            action=Marker.ADD,
            scale=Vector3(x=0.05, y=1.0, z=0.0),  # Set the scale of the line 
            color=ColorRGBA(r=0.0, g=0.0, b=0.0, a=1.0)  # Set the color of the line 
        )

        # Set the pose (position and orientation) of the marker 
        marker_msg.pose = Pose()
        marker_msg.pose.orientation = Quaternion(*tf.transformations.quaternion_from_euler(0, 0, 0))  # Set the orientation to identity quaternion

        # Populate the points of the marker from the trajectory
        marker_msg.points = [Point(x=point[0], y=point[1], z=0.0) for point in trajectory]

        marker_array_msg = MarkerArray() 
        marker_array_msg.markers.append(marker_msg) 

        # Publish the MarkerArray message
        self.pub_trajectory.publish(marker_array_msg) 
